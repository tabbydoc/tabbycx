...
COLOR_STYLE_PROPERTIES = ['color', 'background-color', 'border-left-color', 'border-bottom-color', 'border-right-color',
                          'border-top-color', 'outline-color']
CATEGORICAL_STYLE_PROPERTIES = ['text-align', 'font-family', 'text-transform', 'text-decoration', 'vertical-align',
                                'display', 'tag']
NUMERIC_STYLE_PROPERTIES = {'border-bottom-width': (-40, 40), 'border-left-width': (-40, 40),
                            'border-right-width': (-40, 40), 'border-top-width': (-40, 40), 'font-size': (0, 60),
                            'padding-bottom': (-40, 40), 'padding-left': (-40, 40), 'padding-right': (-40, 40),
                            'padding-top': (-40, 40), 'font-weight': (400, 700)}
...
DENSITY_SYNTAX_PROPERTIES = {'lowercase': r'\p{Ll}', 'uppercase': r'\p{Lu}', 'alphanumeric': r'\w', 'digit': r'\d',
                             'whitespace': r'\s', 'symbol': r'[^\w\s]'}
...
BOOLEAN_SYNTAX_PROPERTIES = {'capitalised': r'(\p{Lu}(\p{Ll}+|\.)\s)*\p{Lu}(\p{Ll}+|\.)', 'allcaps': r'\p{Lu}+',
                             'money': r'[\$£]\s*\d+([.,] ?\d+)?\s*$|\d+([.,] ?\d+)?\s*€',
                             'amount': r'([\-\+]\s*)?[\d ,\.]+', 'range': r'((\+\-)?\s*\d[\d(,\s?)\.]*[\-\–(to),\s]*)+',
                             'empty': r''}
...
PROPERTY_KINDS = {
    'style': ['background-color-b', 'background-color-g', 'background-color-r', 'border-bottom-color-b',
              'border-bottom-color-g', 'border-bottom-color-r', 'border-bottom-width', 'border-left-color-b',
              'border-left-color-g', 'border-left-color-r', 'border-left-width', 'border-right-color-b',
              'border-right-color-g', 'border-right-color-r', 'border-right-width', 'border-top-color-b',
              'border-top-color-g', 'border-top-color-r', 'border-top-width', 'color-b', 'color-g', 'color-r',
              'display', 'font-family', 'font-size', 'font-weight', 'outline-color-b', 'outline-color-g',
              'outline-color-r', 'padding-bottom', 'padding-left', 'padding-right', 'padding-top', 'text-align',
              'text-decoration', 'text-transform', 'vertical-align'],
    'syntax': ['length', 'density-alphanumeric', 'density-digit', 'density-lowercase', 'density-stopwords',
               'density-symbol', 'density-token', 'density-uppercase', 'density-whitespace', 'match-allcaps',
               'match-amount', 'match-capitalised', 'match-date', 'match-empty', 'match-money', 'match-range'] + [
                  'first-char-%s' % k for k in SYNTAX_NTH_OF_TYPE_PROPERTIES] + ['last-char-%s' % k for k in
                                                                                 SYNTAX_NTH_OF_TYPE_PROPERTIES],
    'structural': ['children', 'colspan', 'rowspan', 'tag', 'col', 'row'],
    'semantic': ['corpus-metadata'] + ['density-postag-%s' % tc for tc in POS_TAG_CATEGORIES]
}
PROPERTY_KINDS = {k: v + ['%s-variability-%s' % (dim, feat) for feat in v for dim in ['row', 'col', 'tab']] for k, v in
                  PROPERTY_KINDS.items()}
...
def segmentate(table, add_image_text, add_link_urls, base_url, normalization='min-max-global', text_metadata_dict=None):
    elements = [[cell for cell in row.select('th,td')] for row in
                table.element.select('tr')]  # td within td and th within th are solved by selenium
...
def place_context(table, add_image_text, add_link_urls, base_url,
                  text_metadata_dict=None):  # TODO test factorization improvement
    ...
    if len(middle_rows):
        ...
        for r in (range(rows) if direction == 'down' else range(rows - 1, -1, -1)):
            ...
            if len(context_row_candidate):
                ...
                context_row = (
                context_row[2], context_row[3], extract_text(context_row[3], add_image_text, add_link_urls, base_url))
            table.features[r].append(context_row[0])
            table.elements[r].append(context_row[1])
            table.texts[r].append(context_row[2])
    table.context = {'_'.join(map(str, k)): extract_text(v, add_image_text, add_link_urls, base_url) for k, v in
                     table.context.items()}
    ...
    for orientation in ORIENTATIONS:
        ...
        for r, row in enumerate(table.features):
            for c, cell in enumerate(row):
                ...
                for feature_type in PROPERTY_KINDS:
                    total_variability[(orientation, feature_type)].append(
                        vector_module({k: v for k, v in variabilities.items() if k in PROPERTY_KINDS[feature_type]}))
...
def normalize(table, normalization='min-max-global'):
    ...
    if normalization == 'min-max-global':
            ...
            for c, cell in enumerate(row):
                ...
                for prefix in ('', 'row-variability-', 'col-variability-', 'tab-variability-'):
                    ...
                    for feat, (start, wide) in NUMERIC_STYLE_PROPERTIES.items():
                        table.features[r][c][prefix + feat] = clamp(
                            (table.features[r][c][prefix + feat] - start) / wide)
        ...
        for kind in PROPERTY_KINDS.values():
            for feat in kind:
                values = [table.features[r][c][feat] for r in range(rows) for c in range(cols) if
                          feat in table.features[r][c]]
            ...
            for feat in kind:
                ...
                for r in range(rows):
                    for c in range(cols):
                        if feat in table.features[r][c]:
                            if tot > 0:
                                table.features[r][c][feat] = exp(
                                    clamp(table.features[r][c][feat], smallest=-PYTHON_EXP_LIMIT,
                                          largest=PYTHON_EXP_LIMIT)) / tot
                            else:
                                table.features[r][c][feat] = 0
...
def functional_analysis(table, clustering_features=['style', 'syntax', 'structural', 'semantic'],
                        dimensionality_reduction='off', clustering_method='k-means'):
    ...
    for kind, feature_names in PROPERTY_KINDS.items():
        ...
        kind_functions[kind] = cluster_vector_table(vector_table, xpath_table, dimensionality_reduction,
                                                    clustering_method)
...
def detect_orientation_table_diff(table):
    groups = [
        [table.features[0][0]],
        table.features[0][1:],
        [r[0] for r in table.features[1:]],
        [c for r in table.features[1:] for c in r[1:]]
    ]
...
def compute_score(table):
    if table.kind == 'unknown':
        ...
        if len(data_area):
            ...
            data_header_score = 1
        element = soup(
            '<table>%s</table>' % ''.join(['<tr>%s</tr>' % ''.join(map(str, row)) for row in table.elements]),
            'html.parser').table
        tokens = [kw.strip() for text in element.find_all(text=True, recursive=True) for kw in text.split(' ')]
        tokens_after = [kw.strip() for rec in table.record for k, v in rec.items() for kw in
                        k.split(' ') + v.split(' ')]
